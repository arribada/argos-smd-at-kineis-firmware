...
static bool is_valid_legacy_command(uint8_t byte)
{
    ...
    switch (byte) {
    /* Reject ONLY the durable identity/credential value-writes as legacy.
     * These corrupt the Argos identity permanently (the mirror/intent hook
     * makes a garbage write survive resets), so a byte-misaligned A+ frame must
     * never execute them via the payload-less legacy path.
     *
     * NOTE (audit #8): the audit-#7 extension that ALSO rejected WRITE_MC/LPM/
     * TCXOWU/TX_SIZE/TX/UDATE/PREPASSEN was REVERTED — its premise ("they carry
     * no payload in legacy") is false. Legacy handlers read their payload from
     * the RAW rxBuf (mgr_spi_cmd.c:168-175, 229-230), so those opcodes DO work
     * as legacy, and rejecting them broke legitimate legacy-SPI TX/config flows
     * (WRITE_TX_REQ -> WRITE_TX_SIZE -> WRITE_TX silently dropped). Their
     * desync hazard is non-durable (MC/LPM/TX are recoverable) and was the
